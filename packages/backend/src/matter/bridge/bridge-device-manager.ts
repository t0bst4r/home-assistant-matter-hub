...
export class BridgeDeviceManager {
  ...
  private async upsertDevices(
    ...
    const entities = allEntities.filter((entity) => {
      ...
      const matchesFilter = matchesEntityFilter(bridgeData.filter, entity) ?? [];
      const reasons = isValid.concat(matchesFilter);
      if (reasons.length) {
        this.log.silly(`Entity ${entity.entity_id} is excluded from ${this.bridgeId}. Reason: ${reasons.join(", ")}`);
        return false;
      }
      return true;
    });
...
function isValidEntity(entity: HomeAssistantEntityInformation): string[] | undefined {
  const reason: string[] = [];
